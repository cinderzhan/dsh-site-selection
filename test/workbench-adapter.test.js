...
function setup({fail=false, wait}={}) {
 let api, current=null, draft='existing', saved=null, descriptor, component
 const state={active:'site-selection',sessionBindings:{}}
 const calls=[]
 const ctx={effect:fn=>fn(), sessions:{list:{getSnapshot:()=>({current}),subscribe:()=>()=>{}},scope:()=>({get:()=>({input:{for:()=>({state:{getSnapshot:()=>({draft})},setDraft:v=>draft=v})}})})},conversation:{},desktopWorkbenches:{getSnapshot:()=>({state}),register:(d,c)=>{descriptor=d;component=c;return()=>{}},ensureSession:async args=>{calls.push(args);if(wait)await wait;if(fail)throw Error('failed');current=args.sessionId||'session-new';state.sessionBindings[current]='site-selection';return current}}}
 const document={head:{appendChild(){}},createElement:()=>({dataset:{}})}
 vm.runInNewContext(source,{window:{__ModuleLoader__:{load:({factory})=>{api=factory(()=>({}))}}},document,localStorage:{getItem:()=>null,setItem(){}},setTimeout,clearTimeout,setInterval,clearInterval,fetch:async(path,options)=>({ok:true,json:async()=>{if(path.endsWith('/projects'))return{projects:[]};if(path.includes('/bootstrap'))return{folder:'/data/project',state:{project:{name:'original',sessionId:saved}}};saved=JSON.parse(options.body).sessionId;return{}}})})
 api.apply(ctx)
