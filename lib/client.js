window.__ModuleLoader__.load({ id: 'dsh-site-selection', factory: (require) => {
  ...
  async function openProject(slug) {
    ...
    if (!sessionId) {
      if (ticket !== projectAction || ownedSession() || bridge.desktopWorkbenches.state.active !== 'site-selection') return
  ...
  function apply(ctx) {
    ...
    ctx.effect(() => ctx.desktopWorkbenches.register({
      ...
      initialization: 'empty', embedded: true, version: '1.1.2', author: 'DataElement', layout: { businessSide: 'left', businessWidth: 0.65 },
    }, Panel))
  }
