window.__ModuleLoader__.load({ id: 'dsh-site-selection', factory: (require) => {
  ...
  .ss-panel{position:relative;width:100%;height:100%;overflow:hidden;display:flex;flex-direction:column;min-width:0;
  ...
  const WORKBENCH_ID = 'site-selection'
  const active = () => bridge?.desktopWorkbenches?.getSnapshot?.().state.active === WORKBENCH_ID
  const canWrite = sessionId => active()
    && bridge.sessions.list.getSnapshot().current === sessionId
    && bridge.desktopWorkbenches.getSnapshot().state.sessionBindings[sessionId] === WORKBENCH_ID
  let selectionTicket = 0
  const pendingDrafts = new Map()

  function fillDraft(sessionId, prompt) {
    if (!canWrite(sessionId)) return false
    const actx = bridge.sessions.scope?.(sessionId)
    const conversation = actx?.get?.('conversation')
    if (!conversation) throw new Error('对话输入不可用')
    const input = conversation.input.for(actx)
    const snapshot = input.state.getSnapshot()
    if (snapshot.occurrences?.length || snapshot.references?.length) throw new Error('请先处理输入框中的引用，再重试')
    const current = snapshot.draft || ''
    if (!current.includes(prompt)) input.setDraft(current.trim() ? `${current}\n\n${prompt}` : prompt)
    return true
  }
  ...
  let linkingSessions = 0
  async function ensureSession(slug, result) {
    linkingSessions += 1
    try {
    const saved = result.state?.project?.sessionId || null
    const sessionId = await bridge.desktopWorkbenches.ensureSession({
      workbenchId: WORKBENCH_ID, folder: result.folder, sessionId: saved,
    })
    ...
    if (sessionId !== saved) {
      ...
      pendingDrafts.set(slug, { sessionId, prompt: onboarding(result) })
    }
    return sessionId
    } finally { linkingSessions -= 1 }
  }

  function flushDraft(slug) {
    const pending = pendingDrafts.get(slug)
    if (pending && openState.project === slug && fillDraft(pending.sessionId, pending.prompt)) pendingDrafts.delete(slug)
  }
  ...
  async function openProject(slug) {
    ...
    if (!active() || ticket !== selectionTicket) return result
    await ensureSession(slug, result)
    if (ticket === selectionTicket && active()) flushDraft(slug)
    return result
  }
  ...
  function Panel() {
    ...
    React.useEffect(() => {
      const onMessage = async event => {
        if (event.origin !== window.location.origin || event.source !== frame.current?.contentWindow || !active()) return
        const data = event.data
        if (data?.project !== openState.project) return
        const ticket = selectionTicket
        if (data?.type === 'dsh-site-selection:ask') {
          try {
            const result = await bootstrap(data.project)
            if (!active() || ticket !== selectionTicket) return
            const sessionId = await ensureSession(data.project, result)
            if (ticket !== selectionTicket || !canWrite(sessionId)) return
            flushDraft(data.project)
    ...
    return h('section', { className: 'ss-panel' },
      h('header', { className: 'ss-head' },
        ...
        h('button', { className: 'ss-icon', type: 'button', title: '关闭', onClick: () => bridge.desktopWorkbenches.leave() }, '×')),
      state.project
  ...
  function apply(ctx) {
    bridge = { sessions: ctx.sessions, conversation: ctx.conversation, desktopWorkbenches: ctx.desktopWorkbenches }
    ...
    request('/api/site-selection/projects')
      .then(r => {
        projectIndex = r.projects.map(row => ({ id: row.id, sessionId: row.sessionId || '' }))
        if (selectionTicket || linkingSessions) return
        const slug = projectForSession(ctx.sessions?.list?.getSnapshot?.()?.current)
        if (active() && slug && slug !== openState.project) { ++selectionTicket; setOpen({ project: slug }) }
        if (active() && slug) flushDraft(slug)
      })
      .catch(() => {})
    ...
    ctx.effect(() => bridge.desktopWorkbenches.subscribe?.(() => {
      if (!active() || linkingSessions) return
      const slug = projectForSession(bridge.sessions.list.getSnapshot().current)
      if (slug && slug !== openState.project) { ++selectionTicket; setOpen({ project: slug }) }
      if (slug) flushDraft(slug)
    }), 'site-selection: follow workbench activation')
    ...
    ctx.effect(() => ctx.desktopWorkbenches.register({
      id: WORKBENCH_ID, title: '门店选址', description: '三维城市地图、点位评分、铺源比较与踩点记录。',
      category: '经营分析', tags: ['选址', '地图'], initialization: 'empty', embedded: true,
      version: '1.2.0', author: 'DataElement', layout: { businessSide: 'left', businessWidth: 0.65 },
    }, Panel), 'site-selection: workbench')

  }

  module.exports = { inject, apply, openProject, ensureSession, fillDraft, subscribe, getState: () => openState }
